...
def check_gpu_availability():
    """Checks for CUDA availability and returns the device string."""
...
def separate_audio(assets_path, audio_path):
    ...
    if audio_extension == 'mp3':
        command = f'--verbose -n mdx_extra --jobs {threads} -d {device_to_use} --mp3 --out "{stems_base}" "{audio_path}"'
    else:
        command = f'--verbose -n mdx_extra --jobs {threads} -d {device_to_use} --out "{stems_base}" "{audio_path}"'
...
def main(args):
    ...
    # Set 'device_to_use' variable value
